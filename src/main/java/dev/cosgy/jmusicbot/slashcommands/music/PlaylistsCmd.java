...
public class PlaylistsCmd extends MusicCommand {
    ...
    public class ShowTracksCmd extends MusicCommand {
        ...
        PlaylistLoader.Playlist playlist = bot.getPlaylistLoader().getPlaylist(guildId, playlistName);
            ...
            if (playlist.getItems().isEmpty()) {
                event.reply(event.getClient().getWarning() + " Playlist `" + playlistName + "` has no tracks.").queue();
                return;
            }

            StringBuilder builder = new StringBuilder(event.getClient().getSuccess() + " Tracks in playlist `" + playlistName + "`:\n");
            for (int i = 0; i < playlist.getItems().size(); i++) {
                builder.append(i + 1).append(". ").append(playlist.getItems().get(i)).append("\n");
            }

            if (builder.length() > 2000) {
                builder.setLength(1997); // Corresponds to Discord's message limit
                builder.append("...");
            }
    ...
    public class MakelistCmd extends MusicCommand {
        ...
        public void doCommand(CommandEvent event) {
            ...
            if (pName == null || pName.isEmpty()) {
                event.replyError("Please enter the playlist name.");
        ...
        public void doCommand(SlashCommandEvent event) {
            if (!checkDJPermission(event.getClient(), event)) {
                event.reply(event.getClient().getWarning() + "Cannot execute due to insufficient permissions.").queue();
                return;
            }
    ...
    public class DeletelistCmd extends MusicCommand {
        ...
        public void doCommand(SlashCommandEvent event) {
            if (!checkDJPermission(event.getClient(), event)) {
                event.reply(event.getClient().getWarning() + "You do not have permission to execute this.").queue();
                return;
            }
            String pname = event.getOption("name").getAsString();
            String guildid = event.getGuild().getId();
            if (bot.getPlaylistLoader().getPlaylist(guildid, pname) == null)
                event.reply(event.getClient().getError() + " The playlist `" + pname + "` doesn't exist.").queue();
            else {
                try {
                    bot.getPlaylistLoader().deletePlaylist(guildid, pname);
    ...
    public class AppendlistCmd extends MusicCommand {
        ...
        public void doCommand(CommandEvent event) {
            ...
            String guildid = event.getGuild().getId();
            if (parts.length < 2) {
                event.reply(event.getClient().getError() + " Please include the playlist name and URL to add.");
                return;
            }
        ...
        public void doCommand(SlashCommandEvent event) {
            if (!checkDJPermission(event.getClient(), event)) {
                event.reply(event.getClient().getWarning() + " Cannot execute due to lack of permissions.").queue();
                return;
            }
    ...
    public class ListCmd extends MusicCommand {
        ...
        public void doCommand(CommandEvent event) {
            ...
            if (!bot.getPlaylistLoader().folderGuildExists(guildId))
                bot.getPlaylistLoader().createGuildFolder(guildId);
            if (!bot.getPlaylistLoader().folderGuildExists(guildId)) {
                event.reply(event.getClient().getWarning() + " Playlist folder does not exist and could not be created.");
                return;
            }
        ...
        public void doCommand(SlashCommandEvent event) {
            ...
            if (!bot.getPlaylistLoader().folderGuildExists(guildId))
                bot.getPlaylistLoader().createGuildFolder(guildId);
            if (!bot.getPlaylistLoader().folderGuildExists(guildId)) {
                event.reply(event.getClient().getWarning() + " Playlist folder does not exist and could not be created.").queue();
                return;
            }
            List<String> list = bot.getPlaylistLoader().getPlaylistNames(guildId);
            if (list == null)
