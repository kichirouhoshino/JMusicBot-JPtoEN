...
public class QueueButtonListener extends ListenerAdapter {
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        if (event.getGuild() == null) {
            event.reply("This operation can only be used within a server.").setEphemeral(true).queue();
            return;
        }

        AudioHandler ah = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
        if (ah == null) {
            event.reply("There is currently no playback queue.").setEphemeral(true).queue();
            return;
        }
